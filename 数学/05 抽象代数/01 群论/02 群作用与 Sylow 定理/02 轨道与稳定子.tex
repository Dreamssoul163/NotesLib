...
$\cdot:G\curvearrowright X$. 对任意的$x\in X$, 它生成的$G$-子集是
\[
...
由轨道-稳定子定理, 如果$\cdot:G\curvearrowright X$是传递的,
那么任取$x\in X$有
\vspace{3pt}
\[
    X=\mathrm{orb}(x)\cong\,\!^{\displaystyle G}\!
    /_{\displaystyle\mathrm{Stab}(x)},\\[3pt]
\]
进一步, 不同的$x\in X$给出的陪集空间$\,\!^{\displaystyle G}\!
/_{\displaystyle\mathrm{Stab}(x)}$之间也是等变同构的.
