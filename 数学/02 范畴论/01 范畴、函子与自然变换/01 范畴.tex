...
\begin{enumerate}
    \item 对任意的$f\in M$定义$s^{\mathrm{op}}(f)=t(f)$, $t^{\mathrm{op}}(f)=s(f)$,
    \item 对任意的$f,g\in M$, 当$s^{\mathrm{op}}(g)=t^{\mathrm{op}}(f)$时
    定义$g\circ^{\mathrm{op}}f=f\circ g$,
\end{enumerate}
则$\mathbf{C}^{\mathrm{op}}=
(O,M,s^{\mathrm{op}},t^{\mathrm{op}},i,\circ^{\mathrm{op}})$是范畴,
...
\begin{enumerate}
    ...
    \item 对任意的$f,g\in M$, 在有定义的情形下, $g\circ'f=g\circ f$,
\end{enumerate}
